...
// 
void TopologyWorker::setPartList(std::vector<TLorentzVector> & e1, std::vector<TLorentzVector> & e2)
{	

  //  std::cout << "now in setPartList" << std::endl;
  ...
  for(Int_t elem=0;elem<numElements2;elem++) {
    ...
    mom2(np2,1) = e2[elem].X();
    mom2(np2,2) = e2[elem].Y();
    mom2(np2,3) = e2[elem].Z();
    mom2(np2,4) = e2[elem].T();
